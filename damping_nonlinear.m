%Response of Non-linear damping pendulum 
function damping_nonlinear()
clc
close all
clear all
w=0.08;
...
f = (cos(w.*t)-delta*(-w.*sin(w*t)))/(1-w^2)+eps;
xw = fft(f);
...
dydt=[y(2);-beta*y(1)-delta*w*sin(w*t)+gamma*cos(w*t)];
ydiff=dydt;
end
function ydiff=damp(t,y)
dydt=[y(2); -beta*y(1)-delta*w*sin(w*t)];
ydiff=dydt;
end
s1 = ifft(x)
figure(1)
